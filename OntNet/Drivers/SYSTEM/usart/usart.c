...
void HAL_UART_MspInit(UART_HandleTypeDef *huart)
{
	...
	if (huart->Instance == USART3_UX)                            /* 如果是串口3，进行串口3 MSP初始化 */
    {
        ...
        HAL_NVIC_SetPriority(USART3_UX_IRQn, 2, 2);              /* 组2，最低优先级:抢占优先级2，子优先级2 */
#endif
    }
}

extern unsigned char esp8266_buf[USART3_REC_LEN];		/* 接收缓冲,最大USART3_REC_LEN个字节.末字节为换行符 */
extern unsigned short esp8266_cnt, esp8266_cntPre;/* 接收状态标记 */
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	...
	if (huart->Instance == USART3_UX)                    /* 如果是串口3 */
    {
		...
		esp8266_buf[esp8266_cnt++] = g_rx_buffer[0];
...
void Usart_SendString(USART_TypeDef *USARTx, unsigned char *str, unsigned short len)
{
	...
	for(; count < len; count++)
	{
		...
		while ((USARTx->SR & 0X40) == 0);     /* 等待上一个字符发送完成 */

		USARTx->DR = *str++;             /* 将要发送的字符 ch 写入到DR寄存器 */	
	}
